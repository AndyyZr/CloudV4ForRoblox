...
if not isfile('newvape/profiles/nofirst.txt') then
	...
	notif('AndyV2', 'nuker == breaker', 4.5)
end

run(function()
	local ItemlessLongjump = {Enabled = false}

	ItemlessLongjump = vape.Categories.CloudWare:CreateModule({
...
run(function()
    ...
    local Shaders = vape.Categories.CloudWare:CreateModule({
        Name = "Shaders",
        Function = function(callback)
            if callback then
...
run(function()
    ...
    KrystalXploit = vape.Categories.CloudWare:CreateModule({
        Name = "KrystalExploit",
        Function = function(callback)
            if callback then
                spawn(function()
...
notif('AndyV2', 'Loaded successfully!', 10)
